...
- **`deepseek`** - DeepSeek API with native thinking and reasoning effort controls [[see example](#deepseek)]
- **`kimi`** - Kimi Code Plan membership API with coding models, preserved thinking, and reasoning effort controls [[see example](#kimi-code-plan)]
...
The `deepseek` provider uses the official Chat Completions endpoint and supports DeepSeek's native `thinking` and `reasoning_effort` request fields:
...
```json
{
  ...
  "deepseek-flash": {
    ...
    "ai:thinking": { "type": "disabled" }
  }
}
```

The provider automatically sets:
- `ai:endpoint` to `https://api.deepseek.com/chat/completions`
- `ai:apitype` to `openai-chat`
- `ai:apitokensecretname` to `DEEPSEEK_KEY`
- `ai:capabilities` to `["tools"]`

Store the key with `wsh secret set DEEPSEEK_KEY=...`. `ai:thinking.type` accepts `enabled` or `disabled`; `ai:reasoningeffort` accepts `low`, `high`, or `max`.

### Kimi Code Plan

The `kimi` provider connects to the Kimi Code Plan membership API. Create a dedicated key in the Kimi Code Console; Kimi Platform pay-as-you-go keys are not interchangeable with Kimi Code Plan keys.
...
The provider automatically sets:
- `ai:endpoint` to `https://api.kimi.com/coding/v1/chat/completions`
- `ai:apitype` to `openai-chat`
- `ai:apitokensecretname` to `KIMI_CODE_KEY`
- `ai:thinking` to `{ "type": "enabled", "keep": "all" }`
- `ai:capabilities` to `["tools", "images"]`
- model switching compatibility between configured Kimi modes

Store the Kimi Code Plan key with `wsh secret set KIMI_CODE_KEY=...`. Model availability depends on the membership tier:

| Model | Availability | Context | `ai:reasoningeffort` |
|---|---|---|---|
| `kimi-for-coding` | All Kimi Code members | 256K | Omit |
| `k3-256k` | Moderato and above | 256K | `low`, `high`, or `max` |
| `k3` | Moderato and above | Up to 1M by plan | `low`, `high`, or `max` |
| `kimi-for-coding-highspeed` | Allegretto and above | 256K | Omit |

Kimi Code models require thinking to remain enabled. The provider preserves `reasoning_content` between turns and sends Wave Terminal's real client identity as required by the Kimi Code usage policy.
...
Modes from the same direct provider can be switched inside an existing chat. Switching between incompatible providers starts with the **New Chat** action so provider-specific reasoning history is not mixed.
...
| `ai:provider` | No | Provider preset: `openai`, `openrouter`, `nanogpt`, `groq`, `google`, `deepseek`, `kimi`, `azure`, `azure-legacy`, `custom` |
...
| `ai:thinking` | No | Native Chat Completions thinking object: `type` is `enabled` or `disabled`; optional `keep` is `all` |
| `ai:reasoningeffort` | No | Native Chat Completions reasoning effort: `low`, `high`, or `max` |
